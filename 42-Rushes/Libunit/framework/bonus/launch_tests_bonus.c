...
static t_status	analyze_bonus(int status)
{
	int	sig;
...
static void	run_child(t_unit_test *lst)
{
	alarm(TIMEOUT_SECONDS);
	_exit(lst->test());
}

static void	run_all_bonus(t_unit_test *lst, t_result_buf *buf)
{
	pid_t	pid;
	int		wstatus;
	int		i;
	...
	while (lst && i < buf->capacity)
	{
		...
		else
		{
			...
			buf->data[i].status = analyze_bonus(wstatus);
		}
...
int	launch_tests(t_unit_test **lst)
{
	...
	if (total == 0)
		return (0);
	buf.data = (t_result *)malloc(sizeof(t_result) * total);
	if (!buf.data)
		return (-1);
	buf.count = 0;
	buf.capacity = total;
	run_all_bonus(*lst, &buf);
	all_ok = print_all_bonus(&buf);
	free(buf.data);
	while (*lst)
	{
		tmp = (*lst)->next;
